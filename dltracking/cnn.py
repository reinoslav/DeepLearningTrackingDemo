...
from .layers import (AffineLayer, Clip, ConvBiasLayer, ConvLayer,
...
class HumanConvNet(Model):
    ...
    def forward_from_to(self, inputs, from_layer_name=None, to_layer_name=None):
        if from_layer_name is None:
            from_layer_name = list(self.layers.keys())[0]
        if to_layer_name is None:
            to_layer_name = list(self.layers.keys())[-1]
        l_from = list(self.layers.keys()).index(from_layer_name)
        l_to = list(self.layers.keys()).index(to_layer_name)
        for l, layer in zip(range(l_from, l_to+1), list(self.layers.values())[l_from:l_to+1]):
